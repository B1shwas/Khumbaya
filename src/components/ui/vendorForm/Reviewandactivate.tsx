        ...
        <ScrollView className="flex-1 px-6" showsVerticalScrollIndicator={false}>
          ...
          <View className="py-4">
            <View className="flex-row justify-between items-center mb-2">
              <Text className="text-[#181114] text-sm font-medium">Step 4 of 4</Text>
  ...
  return (
    <View className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
      ...
      <View className="flex-row items-center gap-3 mb-4 pb-4 border-b border-gray-100">
        ...
        <View>
          <Text className="text-lg font-bold text-[#181114]">Luxe Event Photography</Text>
          <Text className="text-sm text-gray-500">Photography & Videography</Text>
        </View>
      </View>

      {/* Details */}
      <View className="gap-y-3">
        <DetailItem icon="💎" label="Primary Service" value="Full Day Wedding Package" />
      ...
      <TouchableOpacity className="mt-5 w-full py-2 flex-row items-center justify-center">
        <Text className="text-sm font-medium text-gray-500">✎ Edit Details</Text    >
      </TouchableOpacity>
    </View>
  );
};
...
const DetailItem = ({ icon, label, value }: { icon: string, label: string, value: string }) => (
  <View className="flex-row items-start gap-3">
    ...
    <View>
      ...
      <Text className="text-sm font-medium text-[#181114]">
        {value}
      </Text>
    </View>
  </View>
