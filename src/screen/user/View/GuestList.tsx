    ...
    <View className="flex-row items-center gap-4 flex-1 min-w-0">
      ...
      <View className="flex-col justify-center flex-1 min-w-0">
        ...
        {guest.dietaryRestrictions && guest.dietaryRestrictions.length > 0 ? (
          ...
          <Text className="text-sm text-gray-500">No dietary restrictions</Text>
        )}
        {guest.category && (
...
export default function GuestListPage() {
  ...
  const confirmedCount = guestsData.filter((g) => g.status === "Going").length;
  const totalGuests = guestsData.reduce((acc, g) => acc + (g.hasPlusOne ? 2 : 1), 0);
  const confirmedGuests = guestsData
  ...
  return (
    <SafeAreaView className="flex-1 bg-background-light">
      ...
      <TouchableOpacity className="absolute bottom-6 right-6 z-30 w-14 h-14 items-center justify-center rounded-full bg-primary shadow-xl">
