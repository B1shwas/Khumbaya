...
export default function ViewGuestDetail() {
  ...
  const {
    mutate: submitRsvpResponse,
    mutateAsync: submitRsvpResponseAsync,
    isPending,
  ...
  const hasArrivalDate = Boolean(guestDetail?.event_guest?.arrival_date_time);
  const hasDepartureDate = Boolean(
    guestDetail?.event_guest?.departure_date_time
  );

  type CheckState = "notCheckedIn" | "checkedIn" | "checkedOut";
  const currentCheckState: CheckState = hasDepartureDate
    ? "checkedOut"
    : hasArrivalDate
      ? "checkedIn"
      : "notCheckedIn";

  const [selectedCheckState, setSelectedCheckState] =
    useState<CheckState>(currentCheckState);

  useEffect(() => {
    setSelectedCheckState(currentCheckState);
  }, [currentCheckState]);

  const checkStatusLabel =
    currentCheckState === "checkedOut"
      ? "Checked out"
      : currentCheckState === "checkedIn"
        ? "Checked in"
        : "Not checked in";
  const isCheckActionChanged = selectedCheckState !== currentCheckState;
  ...
  const headerTitle =
    guestDetail?.user_detail?.username?.trim() || "Guest Detail";
  const isSaveDisabled =
    isPending || !(hasAssignmentChanges || isCheckActionChanged);
  ...
  const handleSaveAssignments = async () => {
    ...
    if (isCheckActionChanged) {
      if (selectedCheckState === "checkedIn" && !hasArrivalDate) {
        arrivalDateTime = new Date().toISOString();
      } else if (selectedCheckState === "checkedIn" && hasArrivalDate) {
        arrivalDateTime = guestDetail.event_guest.arrival_date_time ?? null;
      }

      if (selectedCheckState === "checkedOut" && !hasDepartureDate) {
        departureDateTime = new Date().toISOString();
      } else if (selectedCheckState === "checkedOut" && hasDepartureDate) {
        departureDateTime = guestDetail.event_guest.departure_date_time ?? null;
      }
    }
    ...
    try {
      const shouldSubmitAssignments =
        hasAssignmentChanges || isCheckActionChanged;
      if (shouldSubmitAssignments) {
        await submitRsvpResponseAsync(payload);
      }
      ...
      if (isCheckActionChanged) {
        const updatedGuestDetail = {
          ...
          event_guest: {
            ...guestDetail.event_guest,
            arrival_date_time:
              arrivalDateTime ?? guestDetail.event_guest.arrival_date_time,
            departure_date_time:
              departureDateTime ?? guestDetail.event_guest.departure_date_time,
          },
        };
        setGuestDetail(updatedGuestDetail);
      }
      ...
      const message =
        ...
        error?.message ||
        "Failed to save guest details. Please try again.";
  ...
  const handleCreateCategory = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Success",
        "Guest category created. Tap Save to apply changes."
      );
    } catch (error: any) {
      const message =
        error?.response?.data?.message ||
                        ...
                        {PRIORITY_OPTIONS.map((option) => {
                          ...
                          return (
                            <Pressable
                              ...
                              onPress={() =>
                                setNewCategoryPriority(option.value)
                              }
                            >
                              <Text
              ...
              >
                ...
                <View className="flex-row items-center mt-2" style={{ gap: 6 }}>
                  ...
                  <Text variant="caption" className="text-slate-500 text-sm">
                    {guestDetail?.user_detail?.phone?.trim() ||
                      "Phone not available"}
                  </Text>
                </View>
              </LinearGradient>
                  ...
                  <View className="mt-6">
                    <View className="flex-row gap-3">
                      {(["checkIn", "checkOut"] as const).map((action) => {
                        ...
                        const isActive = selectedCheckState === state;
                        return (
                          <TouchableOpacity
                            key={action}
                            onPress={() => setSelectedCheckState(state)}
                    ...
                    <Text className="text-center text-slate-500 text-xs mt-2">
                      Current status: {checkStatusLabel}
                    </Text>
                    {isCheckActionChanged ? (
                      <Text className="text-center text-slate-500 text-xs mt-1">
                        Selected action will apply on Save.
                      </Text>
                    ) : null}
                  </View>
                )}
