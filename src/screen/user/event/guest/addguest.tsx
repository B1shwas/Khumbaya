...
const AddGuestScreen = () => {
	...
	const onInvalidSubmit = useCallback((errors: FieldErrors<AddGuestFormValues>) => {
		if (errors.fullName) {
			Alert.alert("Error", errors.fullName.message ?? "Please enter a guest name");
			return;
		}
		if (errors.email) {
			Alert.alert("Error", errors.email.message ?? "Please enter a valid email");
			return;
		}
		if (errors.phone) {
			Alert.alert("Error", errors.phone.message ?? "Please enter a phone number");
			return;
		}
	}, []);
		...
		<SafeAreaView className="flex-1 bg-white">
			...
			<View className="flex-row items-center justify-between border-b border-slate-100 px-4 py-3">
				<TouchableOpacity
					...
					onPress={() => router.back()}
				>
					<MaterialIcons name="arrow-back" size={24} color="#1a1b3a" />
				</TouchableOpacity>
				<Text className="flex-1 text-center text-lg font-bold text-slate-900">Add Guest</Text>
				<TouchableOpacity className="h-10 w-10 items-center justify-center" disabled>
					<MaterialIcons name="more-vert" size={24} color="#cbd5e1" />
				</TouchableOpacity>
			</View>
			...
			>
				<View className="px-6 pb-4 pt-8">
					<Text className="text-3xl font-bold tracking-tight text-[#1a1b3a]">New Guest</Text>
				...
				<View className="flex flex-col gap-6 px-6" style={{ gap: 24 }}>
					...
					<View style={{ gap: 8 }}>
						<Text className="text-sm font-semibold tracking-wide text-[#1a1b3a]">EMAIL ADDRESS</Text>
						<Controller
							control={control}
							name="email"
							rules={{
					...
					<View style={{ gap: 8 }}>
						<Text className="text-sm font-semibold tracking-wide text-[#1a1b3a]">PHONE NUMBER</Text>
						<Controller
							control={control}
							name="phone"
					...
					>
						<View className="flex-row items-center justify-between">
							<View className="flex-row items-center" style={{ gap: 12 }}>
								<View className="rounded-lg p-2" style={{ backgroundColor: "rgba(238,43,140,0.1)" }}>
									<MaterialIcons name="family-restroom" size={20} color="#ee2b8c" />
								</View>
								<View>
									<Text className="font-bold text-[#1a1b3a]">Invite with Family</Text>
									<Text className="text-xs text-slate-500">Allow guest to bring others</Text>
								</View>
							</View>
							<Switch
						...
						{inviteWithFamily && (
							<View className="border-t border-[#ee2b8c]/10 pt-4" style={{ gap: 12 }}>
								<View style={{ gap: 8 }}>
									<Text
								...
								<View className="flex-row items-start" style={{ gap: 8 }}>
									<MaterialIcons name="info" size={14} color="#ee2b8c" style={{ marginTop: 2 }} />
									<Text className="flex-1 text-xs italic leading-relaxed text-slate-500">
										The primary guest will be able to add names and details for each family member
										during their digital RSVP process.
									</Text>
								</View>
							</View>
						)}
				...
				<View className="mt-8 px-6">
					<TouchableOpacity
						...
						style={{ gap: 8, shadowColor: "#ee2b8c", shadowOpacity: 0.3, shadowRadius: 12, elevation: 6 }}
						activeOpacity={0.9}
