...
const buildInitialForm = (draft?: Event | null): EditEventForm => {
    ...
    const normalizedType = draft?.type ? EVENT_TYPE_TO_BACKEND[draft.type as keyof typeof EVENT_TYPE_TO_BACKEND] ?? draft.type : "";
    return {
        title: draft?.title ?? "",
...
export default function EditEventScreen() {
    ...
    const defaultValues = useMemo(() => buildInitialForm(eventDraft), [eventDraft]);
    const { control, setValue, watch, reset, handleSubmit } = useForm<EditEventForm>({
        defaultValues,
    });
    ...
    const handleStartDateChange = (event: DateTimePickerEvent, pickedDate?: Date) => {
    ...
    const handleRsvpDateChange = (event: DateTimePickerEvent, pickedDate?: Date) => {
        if (event.type === "dismissed" || !pickedDate) return;
    ...
    const handleUpdate = handleSubmit((values) => {
        ...
        const resolvedType = EVENT_TYPE_TO_BACKEND[
                ...
                >
                    ...
                    <View className="mt-6 gap-6 px-4">
                        ...
                        <View className="flex-row items-center justify-between rounded-2xl border border-red-200 bg-red-50 p-4">
                            <View>
                                <Text className="text-sm font-bold text-red-600">Cancel Event</Text>
