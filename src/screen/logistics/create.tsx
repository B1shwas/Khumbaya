...
export default function CreateLogisticsScreen() {
    ...
    const onStartChange = (event: any, date?: Date) => {
        ...
        if (date) {
            ...
            if (pickerMode === "date") {
                ...
                setValue("availablityStartTime", newDate, { shouldDirty: true, shouldValidate: true });
                if (Platform.OS === "android") {
                    setShowStartPicker(false); // Close date
                    setTimeout(() => {
                        setPickerMode("time");
                        setShowStartPicker(true); // Open time
                    }, 0);
                } else {
                    setPickerMode("time");
                }
    ...
    const onEndChange = (event: any, date?: Date) => {
        ...
        if (date) {
            ...
            if (pickerMode === "date") {
                newDate.setFullYear(date.getFullYear(), date.getMonth(), date.getDate());
                setValue("availablityEndTime", newDate, { shouldDirty: true, shouldValidate: true });
                if (Platform.OS === "android") {
                    setShowEndPicker(false);
                    setTimeout(() => {
                        setPickerMode("time");
                        ...
                        <View className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm">
                            ...
                            <View className="flex-row items-center justify-between w-full mb-4">
                                <View className="flex-row items-center gap-4">
                                    ...
                                    <Text className="text-base text-zinc-600 font-medium">Start</Text>
                                </View>
                                <View className="flex-row gap-2">
                                    ...
                                    >
                                        <Text className="text-zinc-700 text-sm font-medium">{formatDate(startDate.toISOString())}</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity
                                        onPress={() => { setPickerMode("time"); setShowStartPicker(true); }}
                                        className="px-4 py-2 bg-zinc-100 rounded-full"
                                    >
                                        <Text className="text-zinc-700 text-sm font-medium">{formatTime(startDate.toISOString())}</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                            ...
                            <View className="flex-row items-center justify-between w-full">
                                <View className="flex-row items-center gap-4">
                                    ...
                                    <Text className="text-base text-zinc-600 font-medium">End</Text>
                                </View>
                                <View className="flex-row gap-2">
                                    <TouchableOpacity
                                        ...
                                        className="px-4 py-2 bg-zinc-100 rounded-full"
                                    >
                                        <Text className="text-zinc-700 text-sm font-medium">{formatDate(endDate.toISOString())}</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity
                                        onPress={() => { setPickerMode("time"); setShowEndPicker(true); }}
                                        className="px-4 py-2 bg-zinc-100 rounded-full"
                                    >
                                        <Text className="text-zinc-700 text-sm font-medium">{formatTime(endDate.toISOString())}</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                        </View>
